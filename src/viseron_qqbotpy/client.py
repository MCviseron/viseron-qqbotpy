...
class Client:
    ...
    def run(self, appid: str, secret: str) -> None:
        ...
        try:
            ...
            _log.info("[viseron_qqbotpy] 收到 Ctrl+C，机器人已退出")
    ...
    async def _setup(self, appid: str, secret: str) -> None:
        ...
        try:
            ...
            _log.warning("[viseron_qqbotpy] could not fetch bot profile: %s", exc)
            self._robot = {}
    ...
    async def _run_gateway(self) -> None:
        ...
        _log.info(
            "[viseron_qqbotpy] gateway url=%s shards=%s max_concurrency=%s",
            url,
            shards,
            max_concurrency,
        )
    ...
    async def _get_ws_info(self) -> dict:
        ...
        try:
            ...
            _log.warning("[viseron_qqbotpy] /gateway/bot failed, falling back to /gateway: %s", exc)
    ...
    def ws_dispatch(self, event: str, *args: Any, **kwargs: Any) -> None:
        ...
        if method is None:
            _log.debug("[viseron_qqbotpy] event %s has no handler %s", event, method_name)
            return
