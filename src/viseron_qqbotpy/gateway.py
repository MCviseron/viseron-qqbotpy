...
class GatewayShard:
    ...
    async def run(self, stop_event: asyncio.Event) -> None:
        ...
        while not stop_event.is_set():
            ...
            if self._close_code in _FATAL_CLOSE_CODES:
                _log.error("[viseron-qqbotpy] fatal gateway close code %s, stopping shard", self._close_code)
                break
    ...
    async def _connect_once(self, stop_event: asyncio.Event) -> None:
        ...
        try:
            async with session.ws_connect(self.url) as ws:
                self._ws = ws
                _log.info("[viseron-qqbotpy] shard [%s/%s] connected", self.shard_id, self.shard_count)
                async for message in ws:
                    ...
                    if message.type == WSMsgType.TEXT:
                        ...
                        _log.info(
                            "[viseron-qqbotpy] shard [%s/%s] closed code=%s",
                            self.shard_id,
                            self.shard_count,
                            self._close_code,
                        )
                    ...
                    if self._reconnect_requested:
                        _log.info("[viseron-qqbotpy] server requested reconnect")
                        return
        finally:
    ...
    async def _handle_text(self, raw: str) -> None:
        ...
        if op == OP_HEARTBEAT_ACK:
            _log.debug("[viseron-qqbotpy] heartbeat ack")
            return

        if op == OP_RECONNECT:
            _log.info("[viseron-qqbotpy] server asked to reconnect")
            self._reconnect_requested = True
            return

        if op == OP_INVALID_SESSION:
            _log.warning("[viseron-qqbotpy] invalid session")
            self.session_id = None
            self.seq = None
            self._reconnect_requested = True
            return
    ...
    def _dispatch_event(self, event_type: str, payload: Dict[str, Any]) -> None:
        ...
        if event == "ready":
            ...
            _log.info("[viseron-qqbotpy] shard [%s/%s] resumed", self.shard_id, self.shard_count)

        parser = self.dispatcher.parsers.get(event)
        if parser is not None:
            parser(payload)
        else:
            _log.debug("[viseron-qqbotpy] no parser for event %s", event_type)
        ...
        try:
            while True:
                ...
                try:
                    ...
                    _log.debug("[viseron-qqbotpy] heartbeat send failed: %s", exc)
        except asyncio.CancelledError:
            pass
